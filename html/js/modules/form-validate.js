        ...
        $formElements.each((i, o) => {
            ...
            if (isAlpha && !isEmpty) {
                if (!checkRegex(value, alphaOnlyRegex)) {
                    ...
                    errorMessage = $element.data('error-alpha');
                }
            }
            if ((isAlphaNum || isNotRequired) && !isEmpty) {
                if (!checkRegex(value, alphaNumOnlyRegex)) {
                    elemError = error = true;
                    errorMessage = $element.data('error-alphanum');
                }
            ...
            if (isNumber && !isEmpty) {
                if (!checkRegex(value, numberOnlyRegex)) {
                    elemError = error = true;
                    errorMessage = $element.data('error-number');
                }
            }
            if (isRequired) {
                if (isEmpty) {
                    elemError = error = true;
                    errorMessage = $element.data('error-required');
                }
            ...
            if (isEmail && !isEmpty) {
                ...
                if (!isValidEmail) {
                    elemError = error = true;
                    errorMessage = $element.data('error-email');
                }
            }
